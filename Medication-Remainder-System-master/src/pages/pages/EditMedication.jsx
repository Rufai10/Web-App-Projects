...
import {
  Plus, X, Pill, Repeat, Calendar, Clock, ArrowLeftCircle, Save, Edit as EditIcon
} from 'lucide-react';
...
// --- Main EditMedication Component ---
export default function EditMedication() {
  const { id } = useParams();
  const navigate = useNavigate();

  // ✅ ADDED 'times' to the state to handle editing
  const [medication, setMedication] = useState({
    id: '', name: '', dosage: '', frequency: '', startDate: '', endDate: '', times: [], image: '',
  });

  // Load medication by ID from localStorage
  useEffect(() => {
    const meds = JSON.parse(localStorage.getItem('medications')) || [];
    const found = meds.find((m) => m.id === id);
    if (found) {
      // Ensure 'times' is always an array
      setMedication({ ...found, times: Array.isArray(found.times) ? found.times : [''] });
    } else {
  ...
  // ✅ ADDED Handlers for the dynamic time fields
  const handleTimeChange = (index, value) => {
    const updatedTimes = [...medication.times];
    updatedTimes[index] = value;
    setMedication((prev) => ({ ...prev, times: updatedTimes }));
  };
  ...
  const removeTimeField = (index) => {
    const updatedTimes = medication.times.filter((_, i) => i !== index);
    setMedication((prev) => ({ ...prev, times: updatedTimes }));
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const meds = JSON.parse(localStorage.getItem('medications')) || [];
    const updatedMeds = meds.map((m) => (m.id === id ? medication : m));
    localStorage.setItem('medications', JSON.stringify(updatedMeds));
    navigate('/manage-medications'); // Navigate back to the management page
  };
      ...
      <div className="fixed inset-0 z-20 grid place-items-center overflow-y-auto p-4">
        ...
        >
          <h2 className="text-3xl font-bold text-white text-center mb-8 flex items-center justify-center gap-3">
            <EditIcon className="text-cyan-300" /> Edit Medication
          </h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                    <div className="relative">
                        ...
                        <input type="text" name="name" value={medication.name} onChange={handleChange} required className="w-full p-3 pl-10 rounded-lg bg-white/10 text-white border border-white/20 focus:ring-2 focus:ring-cyan-400 focus:outline-none"/>
                    </div>
                    <div className="relative">
                        <label className="block text-sm font-medium text-white/80 mb-1">Dosage</label>
                        <Pill className="absolute left-3 bottom-3.5 text-white/50" size={20}/>
                        <input type="text" name="dosage" value={medication.dosage} onChange={handleChange} required className="w-full p-3 pl-10 rounded-lg bg-white/10 text-white border border-white/20 focus:ring-2 focus:ring-cyan-400 focus:outline-none"/>
                    </div>
                    <div className="relative">
                        ...
                        <input type="text" name="frequency" value={medication.frequency} onChange={handleChange} required className="w-full p-3 pl-10 rounded-lg bg-white/10 text-white border border-white/20 focus:ring-2 focus:ring-cyan-400 focus:outline-none"/>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    <div className="grid grid-cols-2 gap-6">
                        <div className="relative">
                            ...
                            <input type="date" name="startDate" value={medication.startDate} onChange={handleChange} required className="w-full p-3 pl-10 rounded-lg bg-white/10 text-white border border-white/20 focus:ring-2 focus:ring-cyan-400 focus:outline-none custom-date-input"/>
                        </div>
                        <div className="relative">
                            <label className="block text-sm font-medium text-white/80 mb-1">End Date</label>
                            <Calendar className="absolute left-3 bottom-3.5 text-white/50" size={20}/>
                            <input type="date" name="endDate" value={medication.endDate} onChange={handleChange} required className="w-full p-3 pl-10 rounded-lg bg-white/10 text-white border border-white/20 focus:ring-2 focus:ring-cyan-400 focus:outline-none custom-date-input"/>
                        </div>
                    </div>
                    <div className="bg-black/20 p-4 rounded-xl">
                        ...
                        <div className="space-y-3">
                            <AnimatePresence>
                                {medication.times && medication.times.map((time, index) => (
                                <motion.div 
                                    key={index} 
